...
<form action="<?= site_url('post/edit/' . $detail_post->id . "/"); ?>" method="post" enctype="multipart/form-data" class="flex flex-col items-center mb-[28px]">
  ...
  <div class="px-[28px] py-6 w-full bg-background relative before:absolute before:-bottom-[14px] before:left-0 before:h-[14px] before:w-full before:bg-secondary">
    ...
    $data['value'] = $detail_post->caption;
    ...
    $data['value'] = $detail_post->description;
    $this->load->view('components/input', $data);
    ?>
    <?php
    $data['id'] = 'brand';
    ...
    $data['isTextarea'] = FALSE;
    $data['type'] = 'date';
    $data['value'] = $detail_post->release_date;
    $this->load->view('components/input', $data);
    ?>
    <div class="flex flex-col mb-6 text-primary-dark">
      ...
      <div class="relative w-full aspect-[5/3]">
        ...
        <img class="w-full h-full object-cover object-center" src="<?= site_url('upload/' . $detail_post->filename) ?>" alt="" id="thumbnail">
        <label class="absolute w-12 md:w-16 aspect-square rounded-full flex items-center justify-center bottom-8 right-8 bg-secondary text-background cursor-pointer" for="file">
      ...
      <input id="file" name="file" class="hidden" type="file" onchange="previewThumbnail();">
    </div>
